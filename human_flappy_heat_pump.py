    ...
    zones.append(
        {
            ...
            "heat_loss": random.uniform(
                0.03, 0.06
            ),  # Random heat loss for each zone (consistent per seed)
...
def calculate_heat_loss(heat_loss_rate):
    return (
        heat_loss_rate * (70 - global_outside_air_temp) / 20
    )  # Adjust rate based on global outside temp
...
def draw_room(zone, x, y):
    ...
    if zone["heat_on"]:
        pygame.draw.circle(
            screen, RED, (x + 100, y + 130), 10
        )  # Red if the heat pump is enabled
...
def update_temperature(zone):
    ...
    if zone["heat_on"]:
        ...
        if zone["room_temp"] < zone["desired_temp"] - 1:
            if (
                zone["cycle_timer"] <= 0
            ):  # Ensure the 1 second timer has elapsed before turning on heat
                zone["heating_element_on"] = True
                zone["cycle_timer"] = (
                    30  # Reset cycle_timer to 1 second (30 frames at 30 FPS)
                )
    ...
    time_in_hours = (
        1 / (30 * 3600)
    ) * 360  # Scale by 360 to represent real-life time (6 hours)
...
def draw_real_time_plot():
    ...
    outside_temp_text = font.render(
        f"Outside Temp: {global_outside_air_temp} F", True, BLACK
    )
...
def draw_energy_consumption():
    ...
    energy_text = font.render(
        f"Elapsed Electrical Energy: {round(total_energy_kwh, 2)} kWh", True, BLACK
    )
    screen.blit(
        energy_text, (plot_x, plot_y + plot_height + 20)
...
def draw_timer():
    ...
    screen.blit(
        timer_text, (plot_x, plot_y + plot_height + 60)
    )  # Positioned below the energy consumption


def show_final_screen():
    screen.fill(WHITE)
    # Display final metrics on a new screen
    final_temp_text = font.render(
        f"Final Temp: {int(zones[0]['room_temp'])}F", True, BLACK
    )  # Example for zone 1
    energy_text = font.render(
        f"Elapsed Electrical Energy: {round(total_energy_kwh, 2)} kWh", True, BLACK
    )
    max_kw_text = font.render(
        f"Max Electrical Power Hit: {round(max_kw_hit, 2)} kW", True, BLACK
    )
...
while running:
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            running = False
        if event.type == pygame.MOUSEBUTTONDOWN:
            # Toggle system on/off when clicking in any zone
            ...
            for i, zone in enumerate(zones):
                ...
                if x <= mouse_x <= x + 200 and y <= mouse_y <= y + 150:
                    zone["heat_on"] = not zone[
                        "heat_on"
                    ]  # Toggle the system (fan/heating element) on/off
    ...
    # Draw the real-time power plot, energy consumption, and the countdown timer
    draw_real_time_plot()
